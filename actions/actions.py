...
from actions.api_AI.llms import GenerativeAI
from actions.api_AI.prompts import GenAIPrompts
...
class ActionLLMGenerateResponseCommon(Action):
    ...
    async def run(
        ...
        llm = GenerativeAI()
        prompt = GenAIPrompts.get_generate_prompt(
...
class ActionLLMRephraseResponseCommon(Action):
    ...
    async def run(
        ...
        llm = GenerativeAI()
        prompt = GenAIPrompts.get_rephrase_prompt(
            template=f"utter_{latest_intent}",
            query=latest_query,
            response=response,
            configure_personality=True,
...
# must be triggered. Also, there are some use
...
class ActionLLMRephraseResponseWelcome(Action):
    ...
    async def run(
        ...
        llm = GenerativeAI()
        prompt = GenAIPrompts.get_custom_prompt(
