...
"""
    ...
    def get_generate_prompt(
        ...
        prompt_template = GenAIPrompts.response_generate_prompt_default

        if template:
            if template in list(GenAIPrompts.custom_generate_prompts.keys()):
                prompt_template = GenAIPrompts.custom_generate_prompts.get(template)
        ...
        if configure_personality:
            prompt = _inject_personality(
                personality=GenAIPrompts.personality, prompt=prompt
            )
    ...
    def get_rephrase_prompt(
        ...
        prompt_template = GenAIPrompts.response_rephrase_prompt_default

        if template:
            if template in list(GenAIPrompts.custom_rephrase_prompts.keys()):
                prompt_template = GenAIPrompts.custom_rephrase_prompts.get(template)
    ...
    def get_custom_prompt(
        ...
        prompt_template = GenAIPrompts.custom_prompts.get(template_name, None)
